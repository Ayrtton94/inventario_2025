          ...
          <table class="table table-bordered table-hover align-middle text-center small">
            ...
            <tbody>
              <tr v-for="pendiente in pendientes" :key="pendiente.id">
                ...
                <td>
                  <span v-if="pendiente.user" class="badge bg-dark fs-5">{{ pendiente.user.name }}</span>
                  <span v-else class="text-muted fs-5">Sin asignar</span>
                </td>
                <td>{{ pendiente.id }}</td>
...
export default {
    ...
    methods: {
        ...
        async asignarSeleccionados() {
            try {
            await axios.post('/asignardth/pendientedth', {
        ...
        async loadPendientes(page = 1) {
            try {
                const response = await axios.get(`/listar/pendientedth?page=${page}`);
        ...
        irCrear() {
            window.location.href = '/pendientedth/create';
        },
        irEditar(id) {
        ...
        eliminar(id) {
            Swal.fire({
                ...
                try {
                    await axios.delete(`/pendientedth/${id}`);
